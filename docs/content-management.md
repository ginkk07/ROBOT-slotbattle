...
| 想修改的內容 | 唯一管理位置 |
|---|---|
| 玩家技能、法力成本、效果、稀有度 | `src/game/data/skills.js` |
...
| 預設開局解鎖與技能／道具欄位數 | `src/game/data/player-progression.js` |
...
3. 如果需要新狀態，先在 `statuses.js` 建立狀態。
4. 若要預設開局解鎖，將 ID 加入 `player-progression.js`；若由成就解鎖，寫入 `achievements.js`。
5. 補效果與資料驗證測試。
