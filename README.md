...
- 每回合取得行動點；「投入1點」會直接拉霸，「投入全部」會投入所有剩餘點數，「自行輸入」可指定任意剩餘點數。
...
普通奇遇「尋寶中的鐵匠」只在玩家持有可強化的普通武器時進入抽選。支付 20 枚金幣後可選擇目前持有的長劍、手裡劍、騎士錘或十字弓進行強化，成功率為 60%；若同時交付 1 個磨刀石與 20 枚金幣，成功率改為 100%。強化成功時普通武器會替換為稀有強化版，失敗時武器保留，但已支付的資源不會返還。強化武器不會進入一般掉落或商店。

普通奇遇「冒險者屍體」最多可搜刮 3 次。每次先依序以 25%／50%／75% 判定是否遭遇該地區菁英怪；遇敵時該次不取得戰利品，先前搜刮所得保留。安全時才抽取普通消耗品 30%、普通武器 10% 或 10～40 枚金幣 60%。同一事件取得武器後，後續搜刮移除武器結果，並依消耗品與金幣原權重 30：60 重新抽選。

## 目前玩家技能與道具

玩家最多持有 3 個技能，技能等級上限為 3；沒有升級數值的技能可以只定義 1 級。重複取得持有且未滿級的技能會升級；滿級技能不再進入獎勵池。
開局時可從普通裝備「長劍、幸運草、手裡劍」中選擇 1 件；生命藥水與火焰炸彈仍可在冒險途中取得。
...
| 裝備 | 普通 | 長劍 | 自動生效 | 戰鬥開始時獲得攻擊力＋1，持續 3 回合。 |
...
| 裝備 | 普通 | 手裡劍 | 自動生效 | 每次拉霸固定造成1點額外傷害。 |
| 裝備 | 普通 | 騎士錘 | 自動生效 | 拉霸攻擊成功時，依投入的❇️使敵人獲得等量裝甲破壞。 |
| 裝備 | 普通 | 十字弓 | 自動生效 | 三枚⚔️時，該次拉霸攻擊無視敵方護甲。 |
...
| 裝備 | 稀有 | 長劍(強化) | 自動生效 | 戰鬥開始時獲得攻擊力＋5，持續 3 回合。 |
| 裝備 | 稀有 | 手裡劍(強化) | 自動生效 | 每次拉霸使本回合手裡劍額外傷害＋1，回合結束清零。 |
| 裝備 | 稀有 | 騎士錘(強化) | 自動生效 | 依投入的❇️附加等量裝甲破壞；敵人沒有護甲時，追加等同於該次層數的額外傷害。 |
| 裝備 | 稀有 | 十字弓(強化) | 自動生效 | 三枚⚔️時，該次拉霸攻擊無視敵方護甲並造成雙倍傷害。 |
