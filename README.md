...
1. **Boss**：同一地區前 8 次完成的遭遇不會遇到 Boss，但機率仍會每次累積 5 個百分點，因此第 9 次遭遇的 Boss 機率是 40%。
2. **奇遇**：Boss 判定失敗後，以 20% 機率發生；整場遊戲的第一次遭遇不會是奇遇。奇遇也會增加一次地區進度。
3. **菁英怪**：Boss 與奇遇都未發生時，基礎機率為 12%；此機率保存在地區資料，可由未來內容調整。
4. **普通怪**：其餘情況進入普通戰鬥。

擊敗 Boss 時玩家 HP 會立即回滿，選完獎勵後前往下一地區。本次冒險的地區深度每增加 1，敵人的基礎生命與基礎傷害線性增加 50%；怪物技能數量、技能倍率、施放機率與其他規則不變。
...
每場戰鬥勝利都會獲得金錢，再依怪物階級判定是否掉落最多 3 個獎勵選項。掉落成功時，每個選項會**各自獨立**抽取內容稀有度，再從符合該階級限制的技能與裝備中抽取內容。

| 擊敗單位 | 掉落率 | 掉落內容 | 普通 | 稀有 | 傳說 | 金錢 |
|---|---:|---|---:|---:|---:|---:|
| 普通怪 | 10% | 僅裝備 | 90% | 10% | 0% | 10～20 |
| 菁英怪 | 75% | 裝備／技能 | 70% | 29% | 1% | 15～30 |
...
- 奇遇稀有度：固定為普通 60%、稀有 30%、傳說 10%，不受玩家技能、裝備或戰鬥獎勵加成影響。

同一批候選不會重複。已裝備的物品、目前持有的消耗品與滿級技能會被排除；持有但未滿級的技能可以再次出現，選取後提升 1 級。技能已滿 3 個時不再產生新技能，但仍可抽到未滿級技能。若合格內容不足 3 個，就只顯示實際可產生的數量。

普通奇遇「神秘商店」固定列出 3 件裝備或消耗品，商品稀有度為普通 90%、稀有 9%、傳說 1%，並可付費提升目前持有且未滿級的技能。第一筆交易底價為 38；同一次進店每完成一筆交易，下一筆價格乘以 1.77。每深入一個地區，該次進店底價再乘以 1.66；所有乘算完成後統一無條件進位。購買道具與強化技能共用同一個漲價次數，離開商店後清空。
...
傷害事件以 `DamageSource` 固定分成 `spin`（拉霸）、`extra`（額外）、`curse`（詛咒）、`reflect`（反射）。技能、道具、裝備與持續狀態造成的直接傷害都屬於 `extra`；詛咒與反射不會互相觸發，也不會再次觸發自身。詛咒無視護甲且只受專用詛咒減傷影響；反射會先結算專用反射減傷，再由護甲抵擋，不受抗性或一般減傷影響。
...
| 掉落表 | `src/game/data/loot-tables.js` | 各怪物階級的掉落率、內容類型、稀有度與金錢區間 |
| 商店規則 | `src/game/data/shop-rules.js` | 商品數量、商品稀有度與地區／連續購買價格倍率 |
| 成就庫 | `src/game/data/achievements.js` | 永久成就條件與開局內容解鎖 |
| 玩家進度規則 | `src/game/data/player-progression.js` | 預設解鎖、開局欄位、技能持有與等級上限 |

內容定義皆為版本化唯讀資料，不寫入 D1。奇遇率為20%，首次遭遇不會出現；奇遇稀有度獨立採普通60／稀有30／傳說10，不受戰鬥獎勵稀有度修正影響。神秘泉水、神秘商店、廢棄營地、密封石室、遠古回響與神秘收藏家目前皆已啟用。玩家只會看到奇遇內文，不會看到內部事件名稱。
